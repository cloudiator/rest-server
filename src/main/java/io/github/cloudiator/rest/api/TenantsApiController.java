...
public class TenantsApiController implements TenantsApi {
    ...
    public ResponseEntity<Tenant> createTenant(@ApiParam(value = "Tenant creation request "  )  @Valid @RequestBody Tenant tenant) {
        String accept = request.getHeader("Accept");
        if (accept != null && accept.contains("application/json")) {
            try {
                return new ResponseEntity<Tenant>(objectMapper.readValue("{  \"tenant\" : \"tenant\"}", Tenant.class), HttpStatus.NOT_IMPLEMENTED);
            } catch (IOException e) {
