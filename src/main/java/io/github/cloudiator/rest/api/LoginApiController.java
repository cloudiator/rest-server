...
public class LoginApiController implements LoginApi {
    ...
    public ResponseEntity<Token> login(@ApiParam(value = "User login request "  )  @Valid @RequestBody Login login) {
        String accept = request.getHeader("Accept");
        if (accept != null && accept.contains("application/json")) {
            try {
                return new ResponseEntity<Token>(objectMapper.readValue("{  \"token\" : \"token\"}", Token.class), HttpStatus.NOT_IMPLEMENTED);
            } catch (IOException e) {
