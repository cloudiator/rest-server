...
public class NodeConverter implements TwoWayConverter<Node, NodeEntities.Node> {
  ...
  private final Base64IdEncoder base64IdEncoder = Base64IdEncoder.create();

  @Override
  public Node applyBack(NodeEntities.Node node) {

    Node rest = new Node();
    rest.setNodeId(base64IdEncoder.encode(node.getId()));
  ...
  public NodeEntities.Node apply(Node node) {
    ...
    builder
        .setId(base64IdEncoder.decode(node.getNodeId()))
