 ...
 */
public class CloudCredentialConverter implements TwoWayConverter<CloudCredential, IaasEntities.Credential> {
    ...
    public IaasEntities.Credential apply(CloudCredential cloudCredential) {
        return IaasEntities.Credential.newBuilder().setUser(cloudCredential.getUser()).setSecret(cloudCredential.getSecret()).build();
    }
}
